...
### Remaining
1. **Import UI (Phase 3, planned 2026-06-15 — AGPL server + web).** Bring `md import` into the collaborative workbench: a server endpoint that accepts an uploaded non-Markdown file, runs the existing `MarkItDownImporter` + conversion cache (the seam is already built, Apache-2.0), writes the `sources/*.md`, and reflects it into the live `Y.Doc` source map so it appears instantly; a web "Import file" control in the editor/source pane. *Net-new scope beyond the original PLAN (which only scoped import as a CLI/ingestion "path", §1/§3/§9, and mandates "ship CLI before web", §11) — added at the user's request as the UI complement to the shipped CLI command.*
2. **In-graph auto-import (Phase 3, future).** A non-Markdown file referenced directly in `inputs:`, converted on resolve (cached by content hash) so editing the binary rebuilds downstream — **must path-confine the input** (unlike the explicit `md import`, where the user names the file). The `Importer`/cache seam is designed for this; see SPEC §3.2/§11.
3. **Later hardening (optional):** session cache (avoid per-request DB hit); admin-assigned (vs first-come) workspace registration; agent-in-container; a `transform` needing allow-listed `node_modules`; an example demoing `sandbox: container`; the object-store/CDN backing for shared artifacts (seam in place); multi-org analytics roll-up + CSV export; **per-model retry/backoff before advancing the chain; broader OpenAI ordering prices (only gpt-4o/4o-mini listed today).**

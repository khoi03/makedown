...
### Remaining
1. **Phase 2.4b — sharing**: `md share` + public read-only artifact views (server-rendered route first; object-store/CDN behind a seam). *No billing — see §15 dual-license pivot.*
2. **Later hardening (optional):** session cache (avoid per-request DB hit); admin-assigned (vs first-come) workspace registration; agent-in-container; a `transform` needing allow-listed `node_modules`; an example demoing `sandbox: container`.

...
### Bugfix — build.md reload-scramble (done 2026-06-15, branch `fix/crdt-reconcile-interleave`)
...
- **Fix:** make the open/close **boundaries synchronous** (they run once per room, not per keystroke — the debounced editing path stays async). `liveDoc` now `restoreDocStateSync` + `applySnapshot(readWorkspaceFromDiskSync)` **before** exposing the doc (reconcile happens in isolation → clean text, never merged with live client ops); `onDispose` calls `WorkspacePersistence.flushSync()` before teardown. Added sync twins in `@makedown/sync`: `restoreDocStateSync`/`saveDocStateSync`, `readWorkspaceFromDiskSync`/`materializeToDiskSync`, `flushSync`.
- **TDD:** 3 deterministic regression tests in `main.test.ts` (sync load before exposure; sync flush on dispose; build.md verbatim across close→reopen). **Server 191 / sync 39; full suite green; `lint:deps` clean.**
...
- **Note (follow-up, out of scope):** the live `switchBranch`/`reloadWorkspace` paths still `loadIntoDoc` (delete+insert) on a doc with clients attached; rare + user-initiated, but could interleave similarly — revisit if branch-switch ever scrambles.

### Remaining
1. **In-graph auto-import (Phase 3, future).** A non-Markdown file referenced directly in `inputs:`, converted on resolve (cached by content hash) so editing the binary rebuilds downstream — **must path-confine the input** (unlike the explicit `md import`, where the user names the file). The `Importer`/cache seam is designed for this; see SPEC §3.2/§11.
2. **Later hardening (optional):** session cache (avoid per-request DB hit); admin-assigned (vs first-come) workspace registration; agent-in-container; a `transform` needing allow-listed `node_modules`; an example demoing `sandbox: container`; the object-store/CDN backing for shared artifacts (seam in place); multi-org analytics roll-up + CSV export; **per-model retry/backoff before advancing the chain; broader OpenAI ordering prices (only gpt-4o/4o-mini listed today).**

...
// ── Cost estimation (`md cost --dry-run`) ────────────────────────────────────
 ...
 * Input tokens come from the rendered prompts; output tokens are the `max_tokens`
 * upper bound. Only stale model targets contribute to the total.
 */
...
async function costForTarget(
  ...
  if (step === "map") {
    const items = target.header.over
      ? await safeList(target.header.over, ctx, outputs)
      : [];
...
async function renderForCost(
  ...
  const prompt = await renderTemplate(target.body, ctx, outputs, true, bindings);
  const system = target.header.system
    ? await renderTemplate(target.header.system, ctx, outputs, true, bindings)
    : "";
  return `${system}\n${prompt}`;
}
...
async function safeList(
  ...
  try {
    return parseList(await readRefContent(over, ctx, outputs));
  } catch {
    return [];
  }
}
